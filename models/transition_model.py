...
class TransitionModel:
    ...
    def update(self, data_batch, deterministic=False):
        ...
        pred_diff_means, pred_diff_logvars = predictions
        ensemble_model_stds = pred_diff_logvars.exp().sqrt().detach()
        if deterministic:
            pred_means = pred_diff_means
        else:
            normal_random = torch.normal(mean=0.0, std=1.0, size=pred_diff_means.shape).to(self.device)
        ...
        pred_values_stable = [net(ip) for ip, net in zip(torch.unbind(obs_batch_tile), self.model_valnet_olds)]
        pred_value_stable = torch.stack(pred_values_stable)
        
        log_prob = get_log_prob(pred_means, pred_diff_means, ensemble_model_stds).unsqueeze(-1)
        ...
        (
            ...
            predictions, groundtruths, true_value_stable, true_value_target, 
    ...
    def model_loss(
        self, predictions, groundtruths, true_value=None, true_value_target=None, 
        ...
        if true_value_target != None:
            true_value = true_value.view(1, -1, 1).tile((self.model.ensemble_num, 1, 1))
    ...
    def predict(self, obs, act, deterministic=False):
        ...
        pred_diff_means = pred_diff_means.detach()
        ...
        if deterministic:
            ...
            normal_random = torch.normal(mean=0.0, std=1.0, size=pred_diff_means.shape).to(self.device)
            pred_diff_means = pred_diff_means + normal_random * ensemble_model_stds
        ...
        if penalty_coeff != 0:
            if not penalty_learned_var:
                ...
                mean_obs_means = torch.mean(ensemble_means_obs, dim=0)  # average predictions over models
                ...
                dists = torch.norm(diffs, dim=2)  # distance in obs space
                penalty = torch.max(dists, dim=0)[0]  # max distances over models
            else:
                penalty = torch.max(torch.norm(ensemble_model_stds, dim=2), dim=0)[0]
            penalized_rewards = rewards - penalty_coeff * penalty
        else:
            penalized_rewards = rewards
        
        assert (type(next_obs) == torch.Tensor)
    ...
    def load_model(self, model_save_dir, name):
        if not os.path.exists(model_save_dir):
            print(f"The path {model_save_dir} does not exist!")
        model_file = os.path.join(model_save_dir, name)
        model_params = torch.load(model_file)
        self.model.load_state_dicts(model_params)
